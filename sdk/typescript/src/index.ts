...
export class LcpClient {
  ...
  async uploadPublicationFromFile(
    filePath: string,
    title?: string,
  ...
  listAdminLicenses(): Promise<{ licenses: AdminLicense[] }> {
    return this.request<{ licenses: AdminLicense[] }>(
  ...
  listAudit(limit?: number): Promise<{ entries: AuditEntry[] }> {
    ...
    return this.request<{ entries: AuditEntry[] }>(
  ...
  async getLicenseStatusDocument(
    licenseID: string,
  ...
  private authHeaders(): HeadersInit {
    if (!this.token) {
      throw new Error(
        "Client token is not set. Call login() first or provide a token.",
      );
    }
    return {
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      throw new Error(
        `${path} returned ${response.status} ${response.statusText}: ${await response.text()}`,
      );
    }
